...
//       setEmailSubject(`Process Update: ${immediateNextStep.PROCESS}`);
//       setEmailMessage(`Dear Team,\n\nPlease find the update for the process: ${immediateNextStep.PROCESS}\n\nPlant: ${formData?.plant}\nApply Date: ${formData?.applyDate}\n\nComments: ${formData.comments}\n\nBest Regards`);
...
import { toast } from "react-toastify";
import ProjectInfoHeader from "./ProjectInfoHeader";
import EmailSelectionModal from "./EmailSelectionModal"; // Import the new component
  ...
  useEffect(() => {
    if (steps.length > 0 && storeData.length > 0) {
      const completedProcesses = storeData
        .filter((item) => item.UPDATED === "YES")
        .map((item) => item.PROCESS);
console.log(storeData,"next stepppppppppppppppp11", completedProcesses);
      const nextStep = steps.find(
        (step) => !completedProcesses.includes(step.PROCESS)
      );
console.log("next stepppppppppppppppp", nextStep);
      if (nextStep) {
        setImmediateNextStep(nextStep);
        setImmediateNextStepIndex(steps.indexOf(nextStep));
      } else {
