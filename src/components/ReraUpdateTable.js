...
// import { Nav, Form, Button, Row, Col, Badge, OverlayTrigger, Tooltip } from "react-bootstrap";
...
//   // 'immediateNextStep' is the TRUE active step determined on load. It's our source of truth.
...
//   console.log(name,"naaaaaaaaaaaaaa",value);
//       try {
...
//   console.log('select planttttttttttttttttttt', selectedPlant,"hhhhhhhhh",viewedStepDetails?.APPLY_DT,"eeeeee",viewedStepDetails?.COMMENTS)

//   const handleConfirmSubmit = async (emails) => {
//     // This logic remains mostly the same, but it's crucial that it uses
//     // `immediateNextStep` for the payload, not `viewedStep`.
//     const isStep2Incomplete = immediateNextStepIndex === 1 && activeSubLevelIndex < SUB_LEVELS.length;
//     if (isStep2Incomplete) {
//         Swal.fire("Incomplete Step", "You must complete all 4 sub-levels for 'Status of the Application' before updating.", "warning");
//         return;
//     }
...
//     const payload = { loc: selectedPlant,  applyDate: viewedStepDetails?.APPLY_DT,  process: immediateNextStep.PROCESS,  emails: emails };
//     const apiUrl = `${API_BASE_URL}/rera-update`; // Make sure endpoint is correct

//     try {
//       await axios.post(apiUrl, payload);
//       await Swal.fire({ icon: "success", title: "Status Updated!", text: `Step '${immediateNextStep.PROCESS}' has been marked as complete.`, timer: 2000, showConfirmButton: false });
//       setSelectedPlant(""); // This will trigger the main useEffect to reset the whole component
//     } catch (error) {
...
//     if (!viewedStepDetails || !viewedStepDetails.UPLOAD_DOC) { return <p className="text-muted mb-0">No documents found for this step.</p>; }
//     let documents = [];
//     try {
...
//                 // A step is clickable if it's completed or the current active one.
...
//                       <Nav.Link
//                         // ✅ NEW: Highlighting the currently viewed step
...
//             {/* The form title now comes from the step being viewed */}
//             {viewedStep ? (
...
//             <Row className="mb-3">
//               <Col md={6}>
//                 <Form.Group><Form.Label>Plant</Form.Label>
//                   <Form.Select name="loc" value={selectedPlant} 
...
//                 <>
//                   <Col md={3}><Form.Group><Form.Label>From Date</Form.Label><Form.Control type="date" value={viewedStepDetails?.FROM_DT || ""} disabled /></Form.Group></Col>
//                   <Col md={3}><Form.Group><Form.Label>To Date</Form.Label><Form.Control type="date" value={viewedStepDetails?.TO_DT || ""} disabled /></Form.Group></Col>
//                 </>
//               ) : (
//                 <Col md={6}><Form.Group><Form.Label>Application Date</Form.Label><Form.Control type="date" value={viewedStepDetails?.APPLY_DT || ""} disabled /></Form.Group></Col>
//               )}
//             </Row>
//             <Row className="mb-3">
//               <Col md={12}><Form.Group><Form.Label>Comments</Form.Label><Form.Control as="textarea" rows={2} value={viewedStepDetails?.COMMENTS || ""} disabled /></Form.Group></Col>
//             </Row>
...
//                     {/* The tooltip logic is now more detailed */}
...
//                   <Button
//                     variant="success" size="lg" onClick={handleEmailSubmit}
//                     disabled={!canUpdate || isStep2Incomplete}
//                     style={(!canUpdate || isStep2Incomplete) ? { pointerEvents: "none" } : {}}
//                   >
...
//             <div className="flex-grow-1 overflow-auto">{renderDocumentHistory()}</div>
//           </div>
//         </Col>
//       </Row>
...
import React, { useEffect, useState, useMemo, useContext } from "react";
import { Nav, Form, Button, Row, Col, Badge, OverlayTrigger, Tooltip, Card } from "react-bootstrap";
import axios from "axios";
import Swal from "sweetalert2";
import { API_BASE_URL, API_DOC_URL } from "../config/Config";
import FormHeader from "./Header";
...
const ReraUpdateTable = () => {
  ...
  console.log(immediateNextStep?.PROCESS,"uuuuuuuuuuuuuuu",viewedStepDetails)
