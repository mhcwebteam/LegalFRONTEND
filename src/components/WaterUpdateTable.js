...
//     if (isFirstProcess) {
//       payload.append("noOfFlats", formData.noOfFlats || "" || null);
...
//     try {
//     const result =  await axios.post(`${API_BASE_URLS}/water-update`, payload, {
...
//       setFormData({
//         loc: selectedPlant, // Keep plant selection but clear other fields
//         applyDate: "",
//         comments: "",
//         noOfFlats: "",
//         KLD: "",
...
//             {/* Dynamic form fields based on selected process or next step */}
//             {renderFormFields()}
...
const WaterUpdateTable = () => {
  ...
  // Updated handleEmailSubmit - just validates and shows modal
  const handleEmailSubmit = () => {
    const newErrors = {};
    if (!formData.loc) newErrors.loc = "Plant selection is required";
    if (!formData.applyDate) newErrors.applyDate = "Apply date is required";
  ...
  // New function to handle email selection from modal
  const handleEmailModalSubmit = (emails) => {
    console.log('✅ Selected emails:', emails);
    setSelectedEmails(emails);
    setShowEmailModal(false);
    
    // Now proceed with the actual form submission confirmation
    setConfirmOpen(true);
  };
