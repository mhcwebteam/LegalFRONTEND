...
//       setLinkDocs([]); setLandDocs([]); setOthDocs([]); setFeasibilityDocs([]);
...
//                       max={new Date().toISOString().split("T")[0]}   // << LIMIT HERE
...
//                             inline
//                             label="Yes"
//                             name="STATUS"  // Changed from "status" to "STATUS"
//                             type="radio"
//                             value="YES"
//                             checked={formData.STATUS === "YES"}  // Changed from status to STATUS
...
//                             type="radio"
//                             value="NO"
//                             checked={formData.STATUS === "NO"}  // Changed from status to STATUS
//                             disabled={!formData.plant}
//                             onChange={handleChange}
//                           />
...
const AirportModifyTable = () => {
  ...
  const validateDocuments = () => {
    ...
    // Validate feasibilityDocs if any files are selected
    ...
    if (linkDocs.length > 0) {
      ...
      if (invalidFiles.length > 0) {
        newErrors.linkDocs = "Only PDF files are allowed for Link Documents";
        isValid = false;
      }
    }

    // Validate landDocs if any files are selected
    if (landDocs.length > 0) {
      const invalidFiles = landDocs.filter(file => !validateFileType(file));
      if (invalidFiles.length > 0) {
        newErrors.landDocs = "Only PDF files are allowed for Land Documents";
        isValid = false;
      }
    }

    // Validate othDocs if any files are selected
    if (othDocs.length > 0) {
      const invalidFiles = othDocs.filter(file => !validateFileType(file));
      if (invalidFiles.length > 0) {
        newErrors.othDocs = "Only PDF files are allowed for Other Documents";
        isValid = false;
      }
    }
  ...
  const handleEmailSubmit = (e) => {
    e.preventDefault();
    if (!validateForm()) {
      if (errors.feasibilityDocs || errors.linkDocs || errors.landDocs || errors.othDocs) {
  ...
  const handleConfirmSubmit = async (emails) => {
    ...
    landDocs.forEach((file) => {
      if (validateFileType(file)) {
        payload.append("land_docs[]", file);
      }
    });

    othDocs.forEach((file) => {
      if (validateFileType(file)) {
        payload.append("oth_docs[]", file);
      }
    });
  ...
  const handleAmendmentUpdate = async (dataFromPanel) => {
    ...
    const allAmendDocs = [...amendLinkDocs, ...amendLandDocs, ...amendOthDocs];
    const invalidAmendFiles = allAmendDocs.filter(file => !validateFileType(file));
    ...
    amendLandDocs.forEach((file) => {
      if (validateFileType(file)) {
        payload.append("land_docs[]", file);
      }
    });

    amendOthDocs.forEach((file) => {
      if (validateFileType(file)) {
        payload.append("oth_docs[]", file);
      }
    });
  ...
  return (
    <>
            ...
            <div style={{
              ...
              <Form className="p-3 border rounded bg-light" style={{
                ...
                <div style={{
                  ...
                  paddingRight: "5px" // Optional: to prevent scrollbar overlapping
                }}>
                  <Row className="mb-3">
                    <Col md={6}>
                      <Form.Group>
      ...
      <WaterDocUploadModal
        ...
        showLandDocs={false}
        showOthDocs={false}
        // Add file validation to the modal
