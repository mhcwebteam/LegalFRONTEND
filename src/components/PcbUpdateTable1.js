...
const PcbUpdateTable = () => {
  ...
  const showEmailModalForUpdate = async (type, processInfo, amendCategory = '') => {
    try {
      ...
      // CONSOLE LOG HERE
      console.log("🚀 Data being sent to AmendUpdateModal:", dataToSend);
...
{/* raja 30/12/2025 */}
                      ...
                      {amendCategories?.map(cat => {
                        ...
                        // 1. Get the last updated row for this specific category
const lastIdxForThisCat = lastAmendedIndexMap[cat] ?? -1;

// 2. Check if this specific row is the "Next" one
const isNextStepForThisCat = index === lastIdxForThisCat + 1;

// 3. The Condition
const canUpdateAmendment = !isAmendUpdated && isLatestAmendmentForCat && isNextStepForThisCat;
...
<td>
  {isAmendUpdated ? (
    ...
    <button
      className="btn btn-warning btn-sm"
      onClick={async () => {
        // Define the specific keys for this category
        const docPath = storeInfo?.[`${cat}_DOC_PATH`];
        const comments = storeInfo?.[`${cat}_COMMENTS`];

        // Validation logic for Amendment Docs and Comments
        const hasValidDocs = docPath && docPath !== '[]' && docPath.trim() !== '' && docPath !== 'null';
        const hasValidComments = comments && comments !== '[]' && comments.trim() !== '' && comments !== 'null';

        // if (!hasValidDocs || !hasValidComments) {
        if (!hasValidComments) {
          await Swal.fire({
            icon: "warning",
            title: "Missing Amendment Data",
            text: `Please ensure ${cat} Comments are available before updating.`,
          });
