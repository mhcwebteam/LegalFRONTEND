...
//  useEffect(() => {
//     const checkRecordsExist = async () => {
//       if (!selectedPlant || !selectedAuthority) {
...
//         // If data exists, set recordsExist to true
//         setRecordsExist(data.length > 0);
//       } catch (error) {
...
//     checkRecordsExist();
//   }, [selectedPlant, selectedAuthority]);
...
//       console.log(response,"reeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee1111111111111111111");
...
//   try {
//     const fileList = item.files_list || [];

//     console.log(fileList, "ggggggggggggggggg");

//     // Ensure it's a string
//     const filePath =
//       Array.isArray(fileList) && fileList.length > 0
//         ? fileList[0]                // take the first file
//         : "";
...
import PollutionDocUploadModal from '../components/PollutionDocUploadModal';
import DocumentModal from '../components/DocumentModal';
import ITDocumentModal from '../components/ITDocumentModal';
...
const TaxReturns = () => {
  ...
  const handleView = async () => {
    ...
    try {
      const url = `${BASE_URL}?PLANT=${encodeURIComponent(selectedPlant)}&APPROVAL_AUTHORITY=${encodeURIComponent(selectedAuthority)}`;
      const response = await axios.get(url);
console.log(response,"responmdeddddddddddd!!!!!!!!!!!!!1");
      setReturnsData(response?.data?.ITReturns || []);
      setShowTable(true);
      setShowCreateForm(false);
      setShowAddModal(false);
