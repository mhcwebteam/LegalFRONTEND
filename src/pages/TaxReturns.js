...
const TaxReturns = () => {
  ...
  useEffect(() => {
    const checkRecordsExist = async () => {
      ...
      try {
        const url = `${API_BASE_URL}/tax-returns-selectedplant/filter?PLANT=${encodeURIComponent(selectedPlant)}&APPROVAL_AUTHORITY=${encodeURIComponent(selectedAuthority)}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/it-returns-Store`, formPayload, {
